...
make_triplist_single <- function(v){
 ## forces a single column of a schedule to be a sensible rota---that
 ## is, a randomly ordered permutation of c(1,2,3,...,n,0,0,0,0,0)
    v[order(v)] <- c(rep(0,sum(v==0)),seq_len(sum(v>0)))
    return(v)
}

make_triplist <- function(S){  # forces each column of a schedule to be sensible.
    for(j in seq_len(ncol(S))){
        S[,j] %<>%  make_triplist_single()
    }
    return(S)
}
...
munge_try <- function(S){  # Tries to munge a schedule; might return an inadmissible schedule
    ...
    return(make_triplist(S))
}

## The four functions below are the ones that are called by the
## gradient function: add1(), sub1(), munge(), and swap().
...
sub1 <- function(S){   # randomly remove a farm in a random week
    j <- sample(ncol(S),1)
    S[,j] <- make_triplist_single(sub1_single(S[,j]))
    return(S)
}
